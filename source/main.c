...
void main()
{
    ...
    enum state {no_brake, brake_highSlip, brake_okaySlip} system_state;
    uint8_t brake_pedal;
    uint16_t slip_ratio;
    ...
    while(1)
    {
        ...
        {
            Deactivate_ABS_Indicator();     // control the LEDs
            Stop_Timer0();
            Stop_Timer2();
        }

        else if(brake_pedal == 0 && slip_ratio > 819)   // brake pedal pressed and slip >= 0.8
        {
            Activate_ABS_Indicator();
            Stop_Timer0();
            Stop_Timer2();
        }
